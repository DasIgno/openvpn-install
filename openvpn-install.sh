...
function installUnbound() {
	...
	if [[ ! -e /etc/unbound/unbound.conf ]]; then
		...
		if [[ $OS =~ (debian|ubuntu) ]]; then
			...
			# Configuration
			echo 'interface: 10.42.42.1
access-control: 10.42.42.1/24 allow
hide-identity: yes
hide-version: yes
use-caps-for-id: yes
prefetch: yes' >>/etc/unbound/unbound.conf
			...
			# Configuration
			sed -i 's|# interface: 0.0.0.0$|interface: 10.42.42.1|' /etc/unbound/unbound.conf
			sed -i 's|# access-control: 127.0.0.0/8 allow|access-control: 10.42.42.1/24 allow|' /etc/unbound/unbound.conf
...
ifconfig-pool-persist ipp.txt" >>/etc/openvpn/server.conf
	...
	2) # Self-hosted DNS resolver (Unbound)
		echo 'push "dhcp-option DNS 10.42.42.1"' >>/etc/openvpn/server.conf
		if [[ $IPV6_SUPPORT == 'y' ]]; then
...
push "route 217.160.12.75 255.255.255.255"' >>/etc/openvpn/server.conf
...
verb 3" >>/etc/openvpn/server.conf
	...
	# Script to add rules
	echo "#!/bin/sh
iptables -t nat -I POSTROUTING 1 -s 10.42.42.0/24 -o $NIC -j MASQUERADE
...
ip6tables -I INPUT 1 -i $NIC -p $PROTOCOL --dport $PORT -j ACCEPT" >>/etc/iptables/add-openvpn-rules.sh
	...
	echo "#!/bin/sh
iptables -t nat -D POSTROUTING -s 10.42.42.0/24 -o $NIC -j MASQUERADE
...
function newClient() {
	...
	until [[ ($IPCLIENTSEG -ge 200 && $IPCLIENTSEG -le 253) || $IPCLIENTSEG -eq 0 ]]; do
		...
		if [[ $IPCLIENTSEG =~ ^[0-9]+$ ]]; then
			if [[ $IPCLIENTSEG -ge 200 && $IPCLIENTSEG -le 253 ]] || [[ $IPCLIENTSEG -eq 0 ]]; then
				break
			fi
		fi
		echo "Invalid input. Please enter a number in the range [200-253] or 0 for dynamic ips."
		IPCLIENTSEG=-1
	done
...
function revokeClient() {
	...
	chmod 644 /etc/openvpn/crl.pem
	find /etc/openvpn/ccd/ -maxdepth 1 -name "$CLIENT" -delete
